...
<div class="px-4 py-8 font-sans" dir="rtl">
    ...
    <div class="flex gap-4 mb-8 overflow-x-auto pb-2 border-b border-gray-200">
        ...
        <button onclick="switchTab('support')" id="tab-support" class="px-6 py-3 font-bold text-sm text-gray-500 hover:text-indigo-600 hover:bg-gray-50 rounded-t-lg transition-colors">
            <i class='bx bx-support mr-1'></i> الدعم الفني
        </button>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="space-y-6">
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex flex-col items-center justify-center">
                <h3 class="text-lg font-bold text-slate-700 w-full text-right mb-4" id="chart-title">أفضل 10</h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                    <h3 class="text-lg font-bold text-slate-700 w-full text-right mb-4">مقارنة شاملة للأداء (أفضل 3)</h3>
                    <div class="w-full relative h-64">
...
function renderRanking() {
    ...
    if (sortedData.length === 0) {
        top3Container.innerHTML = '<div class="col-span-3 text-center text-gray-500 py-10 font-bold">لا توجد بيانات متاحة في هذا القسم.</div>';
    ...
    podiumIndices.forEach((rankIndex, i) => {
        ...
        const card = `
            <div class="relative ${isFirst ? 'bg-indigo-50/80 border-indigo-200' : 'bg-slate-50 border-slate-100'} rounded-3xl p-6 border ${isFirst ? 'shadow-indigo-100/50' : ''} flex flex-col items-center justify-center text-center transform transition duration-500 hover:scale-105 shadow-sm ${isFirst ? 'ring-2 ring-indigo-500 ring-offset-4 ring-offset-slate-50' : ''}">
